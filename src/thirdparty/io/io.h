...
public:
    ...
    Endpoint endpoint() const { return { m_Host, m_Port }; }
...
public:
    ...
    // 接受事件
    virtual IIOSession * onAccept( NetType type, uint16_t listenport, sid_t id, const char * host, uint16_t port ) { return nullptr; }
...
private:
    ...
    using ListenContexts = std::vector<ListenContext *>;
    using ConnectContexts = std::vector<ConnectContext *>;

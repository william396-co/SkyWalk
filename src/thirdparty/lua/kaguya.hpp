...
#ifndef KAGUYA_USE_STACK_TRACE
#define KAGUYA_USE_STACK_TRACE 1
#endif
...
/// @brief Trait class that identifies whether T is a std::unordered_map type.
template <class T> struct is_std_unordered_map : false_type {};
template <class K, class V, class H, class E, class A>
struct is_std_unordered_map<std::unordered_map<K, V, H, E, A> > : true_type {};
}
...
inline int stack_trace( lua_State * state ) {
...
inline int lua_pcall_wrap(lua_State *state, int argnum, int retnum) {
  int pos = 0;
#if KAGUYA_USE_STACK_TRACE
  pos = lua_gettop(state) - argnum;
  lua_pushcfunction(state, stack_trace);
  lua_insert(state, pos);
#endif
  int result = lua_pcall(state, argnum, retnum, pos);
#if KAGUYA_USE_STACK_TRACE
...
public:
  template <class Result, class... Args> Result resume(Args &&... args) {
    ...
    return detail::FunctionResultProxy::ReturnValue(thread, result, nresult - (lua_gettop(state) + 1), types::typetag<Result>());
  }
  template <class... Args> FunctionResults operator()(Args &&... args);
...
private:
  ...
  template <typename K, typename V, typename H, typename E, typename A> struct gettableunorderedmap {
    typedef K key_type;
    typedef V value_type;
    std::unordered_map<K, V, H, E, A> &m_;
    gettableunorderedmap(std::unordered_map<K, V, H, E, A> &m) : m_(m) {}
    void operator()(K key, V value) { m_[key] = value; }
  };
...
public:
  ...
  std::unordered_map<K, V, H, E, A> unordered_map() const {
    std::unordered_map<K, V, H, E, A> res;
    foreach_table<K, V>(gettableunorderedmap<K, V, H, E, A>(res));
    return res;
  }

  /// @brief If type is table or userdata, return key value pair.
  /// @return key value pair
  template <typename K, typename V, typename H, typename E> std::unordered_map<K, V, H, E> unordered_map() const {
    return unordered_map<K, V, H, E, std::allocator<std::pair<const K, V> > >();
  }

  /// @brief If type is table or userdata, return key value pair.
  /// @return key value pair
  template <typename K, typename V, typename H> std::unordered_map<K, V, H> unordered_map() const {
    return unordered_map<K, V, H, std::equal_to<K> >();
  }

  /// @brief If type is table or userdata, return key value pair.
  /// @return key value pair
  template <typename K, typename V> std::unordered_map<K, V> unordered_map() const {
    return unordered_map<K, V, std::hash<K> >();
  }
  std::unordered_map<LuaRef, LuaRef> unordered_map() const;
};
...
public:
  UserdataMetatable() {
    ...
    static_assert(is_registerable<class_type>::value ||
                             !traits::is_std_unordered_map<class_type>::value,
                         "std::unordered_map is binding to lua-table by default.If you "
                         "wants register for std::unordered_map yourself,"
                         "please define KAGUYA_NO_STD_MAP_TO_TABLE");
...
/// @ingroup lua_type_traits
/// @brief lua_type_traits for std::unordered_map<K, V, H, E, A>
template <typename K, typename V, typename H, typename E, typename A>
struct lua_type_traits<std::unordered_map<K, V, H, E, A> > {
  typedef std::unordered_map<K, V, H, E, A> get_type;
  typedef const std::unordered_map<K, V, H, E, A> &push_type;
  ...
  static get_type get(lua_State *l, int index) {
    ...
    return LuaStackRef(l, index).unordered_map<K, V>();
  }
  static int push(lua_State *l, push_type v) {
    lua_createtable(l, 0, int(v.size()));
    for (typename std::unordered_map<K, V, H, E, A>::const_iterator it = v.begin();
         it != v.end(); ++it) {
      util::one_push(l, it->first);
      util::one_push(l, it->second);
      lua_rawset(l, -3);

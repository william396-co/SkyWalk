    ...
    with context:
        ...
        for j in range(0, len(config_desc.attrs)):
            ...
            if types and j in types:
                attr_type = types[j]
            if attr_type == attrs.type and str(default) == str(content[j]):
                continue
...
def code_gen_page_config(config_desc, data, page, minIndex, maxIndex):
    context = Writer()
    write_header(context)
    if not is_valid_name(config_desc.name):
        raise Exception('not_valid_name', config_desc.name)
    ...
    with context:
        ...
        for i in range(minIndex, maxIndex):
            ...
            v = data[i][1]
            if isIntId:
                context('\t[' + str(i + 1) + '] = items[' + str(try_fromat_int(k)) + '],\n')
            else:
                context('\t[' + str(i + 1) + '] = items["' + str(k) + '"],\n')
        context('}\n\n\n')
    
    context('return {items, sortedItems}')
    context("\n")
...
def code_gen_file(file_desc):
    for config in file_desc.configs:
        pageLineNum = config.pageLineNum
        lines = len(config.attr_datas.items())
        maxPage = math.ceil(lines / pageLineNum)
        if maxPage <= 1:
            file_bytes = code_gen_config(config)
            _files[config.name] = (file_bytes, len(file_bytes) // 1024)
        else:
            page_datas = []
            for (k,v) in config.attr_datas.items():
                page_datas.append([k, v])
            context = Writer()
            for i in range(1, maxPage + 1):
                minIndex = (i - 1) * pageLineNum
                maxIndex = min(i * pageLineNum, lines)
                file_bytes = code_gen_page_config(config, page_datas, i, minIndex, maxIndex)
                _files[config.name + "_" + str(i)] = (file_bytes, len(file_bytes) // 1024)
                if i == 1 :
                    context('local pages{0} = require "Config/GameConfig/{1}_{0}"\n'.format(i, config.name))
                    context('local item{0} = pages{0}[1]\n'.format(i))
                    context('local sortedItems{0} = pages{0}[2]\n'.format(i))
                else:
                    context('local pages{0} = require "Config/GameConfig/{1}_{0}"\n'.format(i, config.name))
                    context('local item{0} = setmetatable( pages{0}[1], {{ __index = item{1}}})\n'.format(i, i - 1))
                    context('local sortedItems{0} = setmetatable( pages{0}[2], {{ __index = sortedItems{1}}})\n'.format(i, i - 1))
            context("local data = {{ Items = item{0}, reloaddata = true, SortedItems = sortedItems{0}}}\n".format(maxPage))
            context("function data:get(id)\n")
            context("    return self.Items[id] or self.Items[tostring(id)]\n")
            context("end\n")
            context("return data")
            context("\n")
            _files[config.name] = (context.getvalue(), 0)
#-----------------------------------------------------------------------------------------
...
def gen_code(request, response, toPath):
    ...
    for k,v in _files.items():
        context = v[0]
        size = v[1]
        print('gen code [%s] file = %s'%(type_name(), k), size)
        file = response.addFile()
        file.name = k.lower() + '.lua'

...
local formatFunLine, formatFunTime, formatFunTimeAVG, formatFunRelative, formatFunCount, divider, nilTime
...
local function rebuildColumnPatterns()
    ...
    outputHeader =
        "| %-" ..
        c.fW .. str .. c.fnW .. str .. c.lW .. str .. c.tW .. str .. c.tW .. str .. c.tW .. str .. c.tW .. str .. c.rW .. str .. c.cW .. "s|\n"
    formatHeader = string.format(outputHeader, "FILE", "FUNCTION", "LINE", "TIME", "AVG", "MIN", "MAX", "%", "#")
    outputTitle = "%-" .. c.fW .. "." .. c.fW .. str .. c.fnW .. "." .. c.fnW .. str .. c.lW .. "s"
    formatOutput = "| %s: %-" .. c.tW .. str .. c.tW .. str .. c.tW .. str .. c.tW .. str .. c.rW .. str .. c.cW .. "s|\n"
    formatTotalTime = "Total time: %f s\n"
    formatFunLine = "%" .. (c.lW - 2) .. "i"
    formatFunTime = "%04.4fs"
    formatFunTimeAVG = "%04.4fms"
...
local onDebugHook = function(hookType)
    local joinTime = getTime()
    if hookType == "call" then
        local information = debug.getinfo(2, "nS")
        local funcReport = functionReport(information)
        local callIndex = funcReport.callIndex + 1
        funcReport.callIndex = callIndex
        funcReport.callTime[callIndex] = joinTime
        funcReport.count = funcReport.count + 1
        stackCount = stackCount + 1
        stackInfos[stackCount] = information
        stacks[stackCount] = getTime() - joinTime
    elseif hookType == "return" then
        local information = stackInfos[stackCount]
        if information then
            local funcReport = functionReport(information)
            local oldTime = funcReport.timer
            local callIndex = funcReport.callIndex
            local callTime = funcReport.callTime[callIndex]
            funcReport.callIndex = callIndex - 1

            local leaveTime = getTime()
            local costTime = (leaveTime - joinTime) + stacks[stackCount]
            local timer = oldTime + (leaveTime - callTime) - costTime
            funcReport.timer = timer
            funcReport.maxtimer = mathMax(timer, funcReport.maxtimer)
            funcReport.mintimer = mathMin(timer, funcReport.mintimer)
            stackCount = stackCount - 1
            if stackCount > 0 then
                stacks[stackCount] = stacks[stackCount] + costTime
            end
        end
    end
end
...
--[[ Functions ]] --[[Attach a print function to the profiler, to receive a single string parameter
...
function module.report(filename)
    ...
    for i = 1, reportCount do
        ...
        if funcReport.count > 0 and funcReport.timer <= totalTime then
            ...
            if printThis then
                ...
                local avgTime = string.format(formatFunTimeAVG, (funcReport.timer / count) * 1000)
                local minTime = string.format(formatFunTimeAVG, (funcReport.mintimer) * 1000)
                local maxTime = string.format(formatFunTimeAVG, (funcReport.maxtimer) * 1000)
                ...
                -- Build final line
                local output = string.format(formatOutput, funcReport.title, timer, avgTime, minTime, maxTime, relTime, count)
    ...
    fileWriter:write(divider)
    fileWriter:write(gBroadMgr:logBroadCount())
    fileWriter:close()
    if printFun ~= nil then

...
local function CreateObjectsReferenceInfoContainer(strObjectName, cObjects)
	-- Create new container.
	local cContainer = {}
	...
	-- Init with object values.
	cContainer.m_strObjectName = strObjectName
	cContainer.m_strAddressName = "objects"

	for k, v in pairs(cObjects) do
		if "string" == type(v) then
			cContainer.m_cObjectExistTag[v] = ("\"" .. tostring(v) .. "\"")
		else
			cContainer.m_cObjectExistTag[v] = GetOriginalToStringResult(v)
		end
	end
...
local function CollectSingleObjectReferenceInMemory(strName, cObject, cDumpInfoContainer)
	...
	if "table" == strType then
		...
		if cExistTag[cObject] and (not cNameAllAlias[strName]) then
			cNameAllAlias[strName] = cObject.id or cObject
		end
	...
	elseif "function" == strType then
		...
		if cExistTag[cObject] and (not cNameAllAlias[cCombinedName]) then
			cNameAllAlias[cCombinedName] = cObject.id or cObject
		end
...
-- strObjectName - The object name reference you want to dump.
-- cObject - The object reference you want to dump.
local function DumpMemorySnapshotObjects(strSavePath, strExtraFileName, nMaxRescords, strObjectName, cObjects)
	-- Check object.
	if not cObjects then
		return
	end

	if (not strObjectName) or (0 == string.len(strObjectName)) then
		strObjectName = "DumpMemorySnapshotObjects"
	end
	...
	-- Create container.
	local cDumpInfoContainer = CreateObjectsReferenceInfoContainer(strObjectName, cObjects)
	local cStackInfo = debug.getinfo(2, "Sl")
	if cStackInfo then
		cDumpInfoContainer.m_strShortSrc = cStackInfo.short_src
		cDumpInfoContainer.m_nCurrentLine = cStackInfo.currentline
...
cPublications.m_cMethods.DumpMemorySnapshotObjects = DumpMemorySnapshotObjects
